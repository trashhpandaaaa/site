...
// Compensation for a vote whose counter update failed AFTER the ledger row was
// inserted. Without this the user's one ledger slot is burned on a vote that
// never counted, and every retry 409s forever. Best-effort by design.
export async function undoVote(supabase, { userId, targetType, targetId }) {
  try {
    await supabase
      .from("user_votes")
      .delete()
